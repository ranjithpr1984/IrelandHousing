...
CREATE TABLE IF NOT EXISTS property_rent_data
  USING DELTA
  LOCATION '/user/hive/warehouse/property_rent_data';

-- COMMAND ----------

-- DBTITLE 1,Data count after scrapping
select nvl(website,'Total') Website,count(1) as Count
from default.property_rent_data
group by website with ROLLUP

-- COMMAND ----------

-- DBTITLE 1,Sample of duplicate data
select county,rent,beds,propertyType,
collect_list(website) as websites,
collect_list(address) as address
from default.property_rent_data
where county='Carlow'
group by county,town,rent,beds,propertyType,lower(regexp_replace(regexp_extract(address,'^(.+?),'),'[^A-Za-z0-9]',''))
...
with dup as (
  select county, rent, lower(town) as town, beds, propertyType, min(website) as website,
         lower(regexp_replace(regexp_extract(address,'^(.+?),'),'[^A-Za-z0-9]','')) addressPart
    from default.property_rent_data
   group by county, rent, lower(town), beds, propertyType,
            lower(regexp_replace(regexp_extract(address,'^(.+?),'),'[^A-Za-z0-9]',''))
  having count(1) > 1 and min(website) != max(website)
)
select nvl(website,'Total') Website,count(1) as Count
  from default.property_rent_data as pd
 where exists(
   select 'x'from dup 
   where pd.county = dup.county and pd.rent = dup.rent and lower(pd.town) = dup.town
...
delete from default.property_rent_data as pd
 where exists(
    select 'x'from dup 
    where pd.county = dup.county and pd.rent = dup.rent and lower(pd.town) = dup.town
      and pd.beds = dup.beds and pd.propertyType = dup.propertyType and pd.website != dup.website
...
drop view if exists property_rent_data_dublin_fix;
create view if not exists property_rent_data_dublin_fix as
select postal_dist as town, pd.address
from default.property_rent_data pd
join dublin_postal_dist dpd on dpd.area = pd.town or instr(pd.address,dpd.area) > 0
where county='Dublin'
and town not like 'Dublin%';

update property_rent_data pd set town = (select max(x.town) from property_rent_data_dublin_fix x where x.address=pd.address)
where county='Dublin'
and address in (select y.address from property_rent_data_dublin_fix y);
...
with valid_town as(
  select county, town from default.property_rent_data
...
select pd1.county,pd1.address, pd1.town, max(vt.town) as proposed_town
  from default.property_rent_data as pd1
...
update default.property_rent_data
...
create view IF NOT EXISTS valid_town as select county, town from default.property_rent_data
  where county!=town
  group by county, town having count(1) >= 4;

create view IF NOT EXISTS invalid_town as select county, town from default.property_rent_data
  where county=town
  group by county, town;
...
update property_rent_data set town='Coolbrock' where county='Wexford' and town='Wellington Bridge';
update property_rent_data set town='Coolbrock' where county='Wexford' and town='Wellingtonbridge';
update property_rent_data set town='Kilmore' where county='Wexford' and town='Kilmore Village';
update property_rent_data set town='Fethard' where county='Wexford' and town='Fethard-On-Sea';

update property_rent_data set town='Annascaul' where county='Kerry' and town='Anascaul';

update property_rent_data set town='Castletownsend' where county='Cork' and town='Castletownshend';

update property_rent_data set town='Curragh' where county='Kildare' and town='The Curragh';

update property_rent_data set county='Westmeath' where county='Roscommon' and town='Athlone';
update property_rent_data set town='Athlone' where county='Westmeath' and town='Athlone East';

update property_rent_data set town='Togher' where county='Cork' and town='Togher (Cork City)';
update property_rent_data set town='Cork City' where county='Cork' and town='Cork';

update property_rent_data set town='Wexford Town' where county='Wexford' and town='Wexford';
update property_rent_data set town='Kildare Town' where county='Kildare' and town='Kildare';
update property_rent_data set town='Sligo City' where county='Sligo' and town='Sligo';
...
--  group by county, town having count(1) < 3
--)
--delete from default.property_rent_data pd
...
--RESTORE table default.property_rent_data to VERSION AS OF 77